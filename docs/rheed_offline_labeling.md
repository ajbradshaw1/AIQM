...
This standalone tool turns an archived Growth Monitor session and one or more
model-prediction CSV files into a local interactive timeline. It is independent
of the live GUI and does not connect to cameras, instruments, or the network.

## Data is distributed separately

This branch contains no laboratory session archive, RHEED frame, prediction
...
`rheed_labeling/model_spec.example.json` and update it for each model. Do not
claim a deployment or evaluation status that its manifest does not support.
...
python scripts\build_rheed_labeling_report.py build `
  --session $Session `
...
```powershell
python scripts\build_rheed_labeling_report.py validate `
