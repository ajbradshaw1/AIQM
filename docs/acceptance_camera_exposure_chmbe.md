# Ch-MBE camera-exposure acceptance — `feature/gui-camera-exposure`
...
|---|---|---|
| `camera_exposure_us` | **`300_000.0`** | `None` |
| Control opens on | **300 ms — ARM writes** | *Keep current* — no write |
| kSA | off (2026-08-05 decision) | normally open, shares the camera |
| Access mode in practice | Full | Read is common |
...
- [ ] The confirmed number is the **readback**. It may differ slightly from
      the request if the device quantises to its increment grid — a small
...
> sustains 1 Hz — if duplicates appear well below the ceiling,
> `_MAX_EXPOSURE_PERIOD_FRACTION` should come down.
...
At 1 Hz against a 300 ms exposure the camera is not over-triggered, so the
GUI must never re-serve a cached frame as a new acquisition.
