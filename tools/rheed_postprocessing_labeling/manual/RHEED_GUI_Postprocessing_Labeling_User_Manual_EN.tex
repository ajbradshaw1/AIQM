...
\fancyfoot[L]{\small\color{ManualMuted}Manual v1.5 | 2026-08-11}
...
Manual v1.5 - English edition\\
Issued 2026-08-11 for the Windows O-MBE and Ch-MBE workstations.\\[2mm]
...
3 & Separate O-MBE and Ch-MBE defaults & Record approved values without cross-copying \\
...
\begin{enumerate}[leftmargin=7mm,itemsep=1.5pt,topsep=1pt]
  \item Confirm the title-selected chamber, advancing RHEED sequence, valid temperature state, intended log directory, and every selected interface against the matching owner-approved register in Chapter~\ref{sec:chamber-defaults}. A pending field is not a default.
...
% Chapter 3 - chamber-specific defaults (reserved)
\manualsection{3}{Keep O-MBE and Ch-MBE defaults separate}

\inbrief{Every operating default is pending until its chamber owner approves it. Keep the two registers physically and logically separate; never infer from code, screenshots, current selections, or the other chamber.}
\label{sec:chamber-defaults}

\begin{dangerbox}{Reserved placeholders - not operating values}
Every value in both registers is intentionally pending. These entries must not be used to configure, ARM, or troubleshoot either chamber until that chamber's owner supplies and approves them. Until then, follow the applicable chamber SOP and verify the active GUI configuration before ARM. Never copy a value from one chamber to the other.
\end{dangerbox}

Chapter~\ref{sec:live-startup} gives fixed launcher names and window titles only so the operator can identify the selected software profile. Those identity checks, source-code values, and visible UI selections do not populate either operating-default register.

\begin{tcolorbox}[
  enhanced,breakable,title={O-MBE owner-approved defaults - reserved},
...
\toprule
\textbf{Configuration item} & \textbf{Owner-approved default} \\
\midrule
GUI launcher and chamber profile & \emph{Pending - to be supplied} \\
RHEED source, mode, source window, ROI, and crop & \emph{Pending - to be supplied} \\
Pyrometer mode, port, baud, RTS, and backend & \emph{Pending - to be supplied} \\
MISTRAL mode, endpoint, ports, schema, and cell map & \emph{Pending - to be supplied} \\
EvapControl mode, window or log source, and schema & \emph{Pending - to be supplied} \\
Classifier package, enable state, and shadow state & \emph{Pending - to be supplied} \\
Save root, filename prefix, and session naming & \emph{Pending - to be supplied} \\
Sampling, continuous capture, and logging intervals & \emph{Pending - to be supplied} \\
\bottomrule
\end{tabularx}

\textbf{Approved by / revision / date:} \emph{Pending - to be supplied}
\end{tcolorbox}

\begin{tcolorbox}[
  enhanced,breakable,title={Ch-MBE owner-approved defaults - reserved},
...
\begin{infobox}{Scope}
A completed value of \textbf{Not applicable} is valid only when approved by the corresponding chamber owner; a blank or pending value is not. Neither register replaces its chamber startup SOP.
\end{infobox}
...
\begin{itemize}[itemsep=1pt,topsep=1pt]
  ...
  \item Chamber, exact window title, launcher name, launcher-log path, environment path, and selected modes. For labeling issues, add report-manifest and annotation-JSON SHA values; raw images are not initially required. Keep sensitive data out of public channels.
\end{itemize}
...
\begin{itemize}[leftmargin=7mm,itemsep=1.3pt,topsep=1pt]
  ...
  \checkitem{Compare every selected interface and logging field only with the approved O-MBE register in Chapter~\ref{sec:chamber-defaults}; stop on any pending or unexplained value.}
...
\begin{itemize}[leftmargin=7mm,itemsep=1.3pt,topsep=1pt]
  ...
  \checkitem{Compare every selected interface and logging field only with the approved Ch-MBE register in Chapter~\ref{sec:chamber-defaults}; stop on any pending or unexplained value.}
...
The prompt pack does not authorize an AI to invent pending defaults or make instrument-control decisions.
