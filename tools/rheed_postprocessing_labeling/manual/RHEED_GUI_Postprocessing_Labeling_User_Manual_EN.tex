...
\fancyfoot[L]{\small\color{ManualMuted}Manual v1.4 | 2026-08-11}
...
\begin{minipage}[t]{0.485\linewidth}
  ...
  \fbox{\includegraphics[width=0.96\linewidth]{assets/screenshots/chmbe_growth_monitor_dummy.png}}
  \endgroup\\[-1mm]
...
4 & Separate Ch-MBE and O-MBE defaults & Record approved values without cross-copying \\
5 & Labeler desktop application & Select inputs, build, open, and validate \\
6-7 & Inputs and safe report build & Produce a complete local report directory \\
8-9 & Segment labeling and display controls & Create traceable labels on actual saved frames \\
10 & Export, import, and validation & Produce canonical JSON and validate fail-closed \\
11-13 & Safety, troubleshooting, and version checks & Handoff and reproduce the work safely \\
\bottomrule
\end{tabularx}
...
\inbrief{Use the dedicated Ch-MBE shortcut, verify the window title and live status before ARM, and close the GUI normally so logs finish writing.}
...
\manualscreenshot[width=0.78\linewidth,height=50mm,keepaspectratio]{assets/screenshots/chmbe_growth_monitor_session.png}
\screenshotcaption{The current Session tab in a hardware-free demo. Its dummy selections are demonstration values, not Ch-MBE or O-MBE production defaults. Confirm the chamber and every selected interface before ARM.}
...
The approved values, when supplied, belong in the chamber-specific record on page~\pageref{sec:chamber-defaults}; a pending field is not a default.
...
% Page 4 - chamber-specific defaults (reserved)
\manualsection{3}{Keep Ch-MBE and O-MBE defaults separate}
\label{sec:chamber-defaults}

\inbrief{Every default is pending until its chamber owner approves it. Keep Ch-MBE and O-MBE records independent and never copy a value across chambers.}

\begin{dangerbox}{Reserved placeholders - not operating values}
Every value on this page is intentionally pending. These entries must not be used to configure, ARM, or troubleshoot either chamber until the chamber owner supplies and approves them. Until then, follow the applicable chamber SOP and verify the active GUI configuration before ARM. Never copy a value from one chamber to the other.
\end{dangerbox}

\subsection{Ch-MBE default configuration}
\rowcolors{2}{ManualPale}{white}
...
RHEED acquisition source and mode & \emph{Pending - to be supplied} \\
Temperature interface and communication settings & \emph{Pending - to be supplied} \\
Other instrument interfaces and connection settings & \emph{Pending - to be supplied} \\
Classifier or model package & \emph{Pending - to be supplied} \\
Sampling, capture, and logging settings & \emph{Pending - to be supplied} \\
\bottomrule
\end{tabularx}

\textbf{Approved by / revision / date:} \emph{Pending - to be supplied}

\subsection{O-MBE default configuration}
\rowcolors{2}{ManualPale}{white}
\begin{tabularx}{\linewidth}{L{66mm}Y}
\toprule
\textbf{Configuration item} & \textbf{Owner-approved default} \\
...
\begin{infobox}{Scope}
A completed value of \textbf{Not applicable} is valid when approved by the chamber owner; a blank or pending value is not. The O-MBE record on this page does not replace an O-MBE startup SOP.
\end{infobox}
...
\inbrief{Preserve the error, time, branch, commit, paths, and launcher log before changing anything. Do not start by deleting environments, resetting Git, or guessing settings.}
...
\inbrief{Record versions and hashes, complete the checklist, and stop whenever instrument state, provenance, or validation cannot be explained.}
