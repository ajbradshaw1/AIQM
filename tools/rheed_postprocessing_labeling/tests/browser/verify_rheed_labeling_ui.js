...
async function main() {
  ...
  try {
    ...
    assert(exported.dataset.dataset_id && exported.dataset.ordered_frame_fingerprint, 'Missing dataset provenance');
    ...
    for (const column of ['event_id', 'original_frame_sha256', 'review_frame_sha256', 'calibration_id', 'equalizer_htr']) assert(csvText.includes(column), `CSV omits ${column}`);
    ...
    const sync = await page.evaluate(() => {
      ...
      return { modelGuideCount: document.querySelectorAll('.arp-panel .selected-guide').length, frameValue: document.getElementById('arp-frame-scrubber-point').value, zoomValue: document.getElementById('arp-zoom-timeline')?.value ?? null, guideSpread: positions.length ? Math.max(...positions) - Math.min(...positions) : null };
    });
    assert(sync.modelGuideCount > 0 && sync.guideSpread <= 3, 'Timeline playheads are not aligned', sync);
