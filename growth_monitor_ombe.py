"""Launch the Growth Monitor configured for the Oxide MBE (O-MBE).

This entry point deliberately overrides an inherited chamber selection.  A
workstation-level ``AIQM_CHAMBER=chmbe`` must never turn the O-MBE desktop
shortcut into a Ch-MBE process.  On Windows it also owns a named mutex for the
whole Qt process lifetime so two O-MBE monitor instances cannot compete for
the same serial, ADS, and capture interfaces.
"""
