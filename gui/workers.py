...
class EvapControlWorker(QThread):
    ...
    def __init__(
        ...
        self.mode = mode
        self.poll_interval = poll_interval
        # Explicit chamber path from MBESystemConfig.  Passing it through to
        # ElogReader prevents its generic cross-workstation auto-detection
        # from selecting another chamber's install.
    ...
    def _create_driver(self):
        if self.mode == "screengrab":
            ...
            return ElogReader(log_dir=self.log_dir or None)
        else:
